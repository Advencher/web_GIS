function updateSpecie(record, updateURL, grid, rowsOnPage, changedRows, cancel, variable){
    ...
    if (!variable.pressedUndoButton) {
        ...
        {
            if (checkForCBNullSpecie(record, changedRows, variable, false))
                grid.updateRow(record.ID, record);
        }

        else {
            if (checkForCBNullSpecie(record, changedRows, variable, true))
                grid.updateRow(record.ID, record);
        }
    }
    $.ajaxSetup({
    ...
    $.ajax({ url:  updateURL, data: {'dataRow': dataForUpdateSpecie(record) }, method: 'POST' })
        .done(function (data) {
            if(!variable.pressedUndoButton) {
                ...
                updateGridSpecie(grid, changedRows, record, variable);
                //console.log(data);
                //$('#response').json(data);
                //grid.reload();
            }
                ...
                console.log("ОТМЕНА ИЗМЕНЕНИЯ updateSpecie");
