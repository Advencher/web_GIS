...
<!-- Map.js -->
<script src="js/common/yandex_map_api.js"></script>
...
<script>
    ...
    $('#btnAcceptFilter').click(function(){
        ...
        $(".selectArea option:selected").each(function(){selectedAreas.push($(this).text());});
        ...
        var stations = selectedStations.join(",");
        var waterAreas = selectedAreas.join(",");
        console.log(gridFT.data().dataSource);
		if(!dataSource.split('?')[1]) gridFT.data().dataSource = dataSource + '?startDate=' + startDate + '&endDate=' + endDate + '&waterAreas=' + waterAreas + '&stations=' + stations ;
		else gridFT.data().dataSource = dataSource + '&startDate=' + startDate + '&endDate=' + endDate + '&waterAreas=' + waterAreas + '&stations=' + stations ;
        gridFT.reload();
    });
    ...
    $("#tab a").click(function(e){
        ...
        if($(this).text() === 'Пробы')
            download('/samples', null);
        if($(this).text() === 'Фитопланктон')
        {
            download('/AllPhytos', null);
        }
        if($(this).text() === 'Фотосинтетические пигменты')
		{
			download('/pigments_tab', null);
			if (typeof pigment_sid !== 'undefined' && pigment_sid !== -1) pigment_sid = -1;
		}
    });
